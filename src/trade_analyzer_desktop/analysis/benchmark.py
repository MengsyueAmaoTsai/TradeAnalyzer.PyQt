...
from datetime import date as Date, datetime as DateTime, timezone as TimeZone, timedelta as TimeDelta
...
class Benchmark:
    ...
    @staticmethod
    def get_historical_prices(benchmark_symbol: BenchmarkSymbol, start_date: Date = Date.min, end_date: Date = Date.today()):
    ...
    @staticmethod
    def daily_returns(prices: Dict[Date, float], start_date: Date, end_date: Date) -> Dict[Date, float]:
        returns: Dict[Date, float] = {}
        periods: List[Date] = list(filter(lambda date: date >= start_date and date <= end_date, list(prices.keys())))
        returns[periods[0]] = 0
