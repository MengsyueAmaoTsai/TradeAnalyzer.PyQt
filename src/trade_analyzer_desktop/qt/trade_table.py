...
class TradeTable(QTableWidget):
    ...
    def current_trade(self) -> Union[Trade, None]:
        return next(
            filter(lambda trade: trade.id == self.current_strategy_id, self.__trades),
            None,
        )
    ...
    def on_detail_action_clicked(self, checked: bool) -> None:
        ...
        self.__trade_detail_dialog: TradeDetialDialog = TradeDetialDialog(
            self.current_trade
        )
    ...
    def set_trades(self, trades: List[Trade]) -> None:
        ...
        for trade in trades:
            ...
            self.setItem(
                row, 8, QTableWidgetItem(str(round(trade.gross_profit_loss, 2)))
            )
            self.setItem(row, 9, QTableWidgetItem(str(round(trade.fee, 2))))
            self.setItem(
                row, 10, QTableWidgetItem(str(round(trade.net_profit_loss, 2)))
            )
            self.setItem(row, 11, QTableWidgetItem(str(round(trade.mae, 2))))
            self.setItem(row, 12, QTableWidgetItem(str(round(trade.mfe, 2))))
            self.setItem(
                row, 13, QTableWidgetItem(str(round(trade.end_trade_drawdown, 2)))
            )
