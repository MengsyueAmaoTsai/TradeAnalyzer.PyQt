...
from PyQt6.QtWidgets import QTableWidget, QTableWidgetItem, QWidget, QHeaderView, QMenu, QMessageBox
...
class BacktestReportTable(QTableWidget):
    ...
    def selected_backtest_reports(self) -> List[BacktestReport]:
        ...
        for row in selected_rows:
            ...
            backtest_report: Union[BacktestReport, None] = BacktestReportRepository.query_by_id(backtest_report_id)
            if backtest_report:
    ...
    def on_as_default_action_clicked(self, checked: bool) -> None:
        report: Union[BacktestReport, None] = BacktestReportRepository.query_by_id(self.current_backtest_report_id)
        ...
        if not result:
            QMessageBox.warning(self, "WARN", "Error on updating strategy default report.")
            return            
    ...
    def on_delete_action_clicked(self, checked: bool) -> None:
        ...
        orders_deleted: bool = OrderRepository.delete_by_strategy_id(f"{report.strategy_id}:{report.id}")
    ...
    def on_analyze_action_clicked(self, checked: bool) -> None:
        ...
        self.__setting_dialog.starting_capital = max(strategy.starting_capital for strategy in strategies)
        self.__setting_dialog.start_date = max(report.start_date for report in self.selected_backtest_reports)
        self.__setting_dialog.end_date = min(report.end_date for report in self.selected_backtest_reports)
    ...
    def set_backtest_reports(self, backtest_reports: List[BacktestReport]) -> None:
        ...
        for report in backtest_reports:
            ...
            self.setItem(row, 2, QTableWidgetItem(QDate(report.start_date.year, report.start_date.month, report.start_date.day).toString("yyyy-MM-dd")))
            self.setItem(row, 3, QTableWidgetItem(QDate(report.end_date.year, report.end_date.month, report.end_date.day).toString("yyyy-MM-dd")))
    # -------------------------------------------------- Private Methods --------------------------------------------------        
