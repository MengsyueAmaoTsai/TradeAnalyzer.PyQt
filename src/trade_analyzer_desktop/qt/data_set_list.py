...
class DataSetList(QListWidget):
    ...
    def checked_items(self) -> List[DataSetListItem]:
        ...
        """
        return list(
            filter(lambda item: item.is_checked and not item.is_benchmark, self.items)
        )
    ...
    def get_item(self, key: str) -> DataSetListItem:
        ...
        """
        item: Union[DataSetListItem, None] = next(
            filter(lambda result: result.key == key, self.__data_set_list_items), None
        )
