...
class StatisticsView(QWidget):
    def __init__(
        ...
        self.__average_winning_trade_duration: QLabel = QLabel(
            "Average Winning Trade Duration:"
        )
        self.__average_lossing_trade_duration: QLabel = QLabel(
            "Average Lossing Trade Duration:"
        )
    ...
    def set_performance(self, performance: StrategyPerformance) -> None:
        self.__total_net_profit.setText(
            f"Total Net Profit: {round(performance.trade_statistics.total_net_profit, 2)}"
        )
        self.__total_number_of_trades.setText(
            f"# of Trades: {round(performance.trade_statistics.total_number_of_trades, 2)}"
        )
        self.__average_winning_trade_duration.setText(
            f"Average Winning Trade Duration: {performance.trade_statistics.average_winning_trade_duration}"
        )
        self.__average_lossing_trade_duration.setText(
            f"Average Lossing Trade Duration: {performance.trade_statistics.average_lossing_trade_duration}"
        )
        self.__max_drawdown.setText(
            f"Max Drawdown: {round(performance.trade_statistics.max_closed_trade_drawdown)}"
        )
        self.__max_drawdown_percent.setText(
            f"Max Drawdown%: {round(max(performance.trade_statistics.drawdown_percent.values()))}"
        )
