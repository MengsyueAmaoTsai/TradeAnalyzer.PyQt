...
class AnalysisWindow(QWidget):
    def __init__(
        ...
        self.__correlation_analysis_window: CorrelationAnalysisWindow = (
            CorrelationAnalysisWindow(results)
        )

        self.__tab: QTabWidget = QTabWidget(self)
        self.__tab.addTab(self.__overview_window, self.__overview_window.windowTitle())
        self.__tab.addTab(
            self.__equity_chart_window, self.__equity_chart_window.windowTitle()
        )
        self.__tab.addTab(
            self.__trade_analysis_window, self.__trade_analysis_window.windowTitle()
        )
        self.__tab.addTab(
            self.__list_of_trades_window, self.__list_of_trades_window.windowTitle()
        )
        self.__tab.addTab(
            self.__correlation_analysis_window,
            self.__correlation_analysis_window.windowTitle(),
        )
