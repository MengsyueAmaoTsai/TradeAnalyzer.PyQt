...
class PositionManager:
    ...
    def open_positions(self) -> List[Position]:
        return list(
            filter(lambda pos: pos.status == PositionStatus.Opened, self.__positions)
        )

    @property
    def closed_positions(self) -> List[Position]:
        return list(
            filter(lambda pos: pos.status == PositionStatus.Closed, self.__positions)
        )

    def on_order_filled(self, e: OrderFilledEvent) -> None:
        open_position: Union[Position, None] = next(
            filter(
                lambda pos: pos.symbol == e.symbol
                and pos.strategy_id == e.strategy_id
                and pos.status == PositionStatus.Opened,
                self.__positions,
            ),
            None,
        )
    ...
    def register_position_closed_callback(
